...
if __name__ == '__main__':
    ...
    for n, Pt_thr in enumerate(Pt_thr_list):
        ...
        print(eff_deep)
        print("Total rate:")
        rate_deep = compute_deepTau_ptdep_rate(taus_rates[0], taus_rates[1], Nev_den, par, Pt_thr=Pt_thr, L1rate=L1rate_bm)
        print(rate_deep)
    ...
    for index_cut, cut in enumerate(["mediumIso", "tightIso"]):
        eff_base = compute_isocut_eff(taus[0], taus[1], gen_taus[0], gen_taus[1], cut+"Abs", cut+"Rel", Pt_thr=Pt_thr)
        rate_base = compute_isocut_rate(taus_rates[0], taus_rates[1], Nev_den, cut+"Abs", cut+"Rel", Pt_thr=Pt_thr, L1rate=L1rate_bm)
        print(f"{cut}:")
        print(eff_base)
        print(rate_base)
